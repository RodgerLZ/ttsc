...
<table class="easyui-datagrid" id="itemParamList" title="商品列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/ttsc-manager/item/param/list',method:'get',pageSize:30,toolbar:itemParamListToolbar">
    <thead>
        <tr>
...
<script>
    ...
    var itemParamListToolbar = [{
        ...
        handler:function(){
        	TAOTAO.createWindow({
        		url : "/ttsc-manager/item-param-add",
        	});
        }
    },{
        text:'编辑',
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的商品规格吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/ttsc-manager/item/param/delete",params, function(data){
            			if(data.status == 200){
